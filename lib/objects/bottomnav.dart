...
class CustomBottomNavigationBarMentor extends StatelessWidget {
  final int currentIndex;
  final Function(int) onTap;

  const CustomBottomNavigationBarMentor(
      {Key? key, required this.currentIndex, required this.onTap})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: BottomNavigationBar(
        ...
        currentIndex: currentIndex,
        onTap: onTap as void Function(int)?,
        backgroundColor: const Color(0xFF4D9A91),
        items: <BottomNavigationBarItem>[
          BottomNavigationBarItem(
            icon: Padding(
              padding: const EdgeInsets.only(bottom: 5),
              child:
                  Image.asset('images/icons/requests.png', width: 30, height: 30),
            ),
            label: 'Mentees',
          ),
          BottomNavigationBarItem(
            icon: Padding(
              padding: const EdgeInsets.only(bottom: 5),
              child: Image.asset('images/icons/content.png',
                  width: 30, height: 30),
            ),
            label: 'Content',
          ),
